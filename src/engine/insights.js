...
export function computePromptMetrics() {
  ...
  if (rows.length === 0) return { correlations: [], avgTurnsToFirstEdit: null, totalSessions: 0 };
  ...
  const avgTurnsToFirstEdit = editable.length
    ? Math.round(editable.reduce((a, r) => a + r.turns_to_first_edit, 0) / editable.length * 10) / 10
    : null;
